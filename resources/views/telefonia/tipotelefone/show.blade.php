    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><a href="{{route('tipotelefone.index')}}">{{$titleModal}}</a></li>
    ...
    <div class="modal-dialog modal-default">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" action="{{route('tipotelefone.update', $registro->id)}}" method="post" id="formEditar">
                    ...
                    <div class="col-sm-12">
                        ...
                        <input type="text" class="form-control" name="nome" value="{{$registro->nome}}" maxlength="20" placeholder="Nome do registro..." tabindex="-1" autofocus required>
                        </div>
                    </div>
                    </div>
                    <div class="row">
                    <div class="col-sm-12">
                        ...
                        <textarea name="descricao" class="form-control" rows="3" maxlength="300" placeholder="Breve descrição...">{{$registro->descricao}}</textarea>
                        </div>
                    </div>
                    </div>
                </form>
...
function deleteData(id){
    ...
    .then((willDelete) => {
        if (willDelete) {
            $.ajax({
                type: "POST",
                url: "{{url('/tipotelefone')}}/" + id,
                data: {_method: 'delete'},
                success: function(data){
                    toastr.success("Registro excluído com sucesso.", "Feito!");
                    window.location.href = "{{url('/tipotelefone')}}";
                },
                statusCode: {
                    404: function() {
